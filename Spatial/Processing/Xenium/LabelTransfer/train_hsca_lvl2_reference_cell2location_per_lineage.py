...
adata = sc.read_h5ad(f'{data_directory}scdata/hsca_merged.h5ad')
...
for lineage in lineages:
    ...
    adata_ref = adata[adata.obs[global_label].isin([lineage])].copy()
    adata_ref = adata_ref[:, adata_ref.X.sum(0) != 0]
    adata_ref = adata_ref[adata_ref.obs[ref_label] != 'nan']
    
    lineage_label = lineage.replace(' ', '-')

    results_folder = f'{data_directory}/scdata/hsca_cell2location_{ref_label}_{lineage_label}/'
