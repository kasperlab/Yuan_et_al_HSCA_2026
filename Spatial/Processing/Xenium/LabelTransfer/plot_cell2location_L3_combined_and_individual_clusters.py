...
for sample in samples:
    ...
    for subfolder in Path(data_directory).iterdir():
        ...
        if subfolder.name.startswith('output'):
            ...
            adata_lin = adata[adata.obs['cell_type'] == 'Keratinocyte'].copy()
            adata_nonlin = adata[adata.obs['cell_type'] != 'Keratinocyte'].copy()
            ...
            adata_lin.obs[ref_label + '_est'].to_csv(subfolder_path / f'{sample}_{subfolder.name}_{ref_label}.csv', index=True)

            adata.obs[ref_label + '_est'] = 'Other'
            adata.obs.loc[adata_lin.obs_names, ref_label + '_est'] = adata_lin.obs[ref_label + '_est'].values

            # fig, ax = plt.subplots(figsize=(6, 6))

            # adata_nonlin.obs['is_Keratinocyte'] = 'False'
            # adata_nonlin.uns['is_Keratinocyte_colors'] = ['#E4DDD4']

            # sc.pl.spatial(adata_nonlin, 
            #             color='is_Keratinocyte', spot_size=10, alpha=0.5, frameon=False,
            #             show=False,
            #             legend_loc = None,
            #             ax = ax)
            ...
            # plt.savefig(figure_directory + f'bogle26_xenium_hsca_{ref_label}_{sample}_{subfolder.name}.pdf',
            #             bbox_inches='tight',
            #             transparent=True)
            ...
            #         plt.savefig(figure_directory + f'bogle26_xenium_hsca_{ref_label}_{sample}_{subfolder.name}_{_label.replace(" ", "-").replace(".", "-").replace("/", "-")}.pdf',
            #                     bbox_inches='tight', transparent=True)
            
            adata.write(subfolder_path / f"adata_proseg_filtered.h5ad", compression="gzip")
