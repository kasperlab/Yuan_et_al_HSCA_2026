...
for lin in lineages:
    ...
    adata_lin = sc.read_h5ad(f'{data_directory}clavel25_ctl_occ_nonadi_{lin_label}_integrated_annotation_L3.h5ad')
    ...
    lvl3_results = adata_lin.obsm['integrated_annotation_L3_cell_abundance_w_sf']

    lvl3_results_norm = lvl3_results.div(lvl3_results.sum(1) + 1e-12, axis=0)

    lvl3_entropy =  -np.nansum(lvl3_results_norm * np.log(lvl3_results_norm + 1e-12), axis=1)
    entropy_max = lvl3_results.shape[1] * np.log(lvl3_results.shape[1])
    lvl3_evenness = lvl3_entropy / entropy_max

    adata_lin.obs['integrated_annotation_L3_est'] = lvl3_results.idxmax(axis=1).str.replace('q05cell_abundance_w_sf_', '').values   

    adata_lin.obs['integrated_annotation_L3_est'].to_csv(f'{data_directory}clavel25_stereoseq_ctl_occ_nonadi_' + lin_label + '_integrated_annotation_L3.csv')
    
    # for _label in sorted(adata_lin.obs['integrated_annotation_L3_est'].unique()):
    #     adata.obs[ _label] = 'False'
    #     adata.obs.loc[adata_lin[adata_lin.obs['integrated_annotation_L3_est'] == _label].obs_names, _label] = 'True'
    ...
    #     plt.savefig(figure_directory + 'clavel25_stereoseq_hsca_integrated_annotation_L3_est_' + _label.replace(' ', '-').replace('.', '-').replace('/', '-')+ '.pdf')
