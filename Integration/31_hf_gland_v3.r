...
#Configuration
##############
py$this_output_dir = paste0(py$output_dir, "/", this_celltype_name, "_", hvg_number, "_hvgs_", pca_dim, "_pcs_Karl")
...
if(file.exists(path_list[["clustering"]]) && file.exists(path_list[["clustering_hvg"]])){
  py_run_string(paste0("adata_", this_celltype_name, "_imbalance = ad.read_h5ad('", path_list[["clustering"]], "')"))
  py_run_string(paste0("adata_", this_celltype_name, "_imbalance_hvg = ad.read_h5ad('", path_list[["clustering_hvg"]], "')"))
  ...
  eval(parse(text = paste0("py$adata_", this_celltype_name, "_imbalance = pca_cal_list[['complete_matrix']]")))
  py$adata_full = pca_cal_list[["complete_matrix"]]
  summary_list[["merge_by"]] = list("merge_by" = unlist(py$merge_by))
  py$adata_full$obsm["PCA_use"] = pca_cal_list[["hvg_matrix"]]$obsm["PCA_use"]
  rm(pca_cal_list)
  # Clustering
  ...
  ####################
  if(use_cache){
    py_run_string(paste0("adata_", this_celltype_name, "_imbalance.write('", path_list[["clustering"]], "')"))
...
#Configuration
##############
py$this_output_dir = paste0(py$output_dir, "/", this_celltype_name, "_", hvg_number, "_hvgs_", pca_dim, "_pcs_Karl_20250203")
dir.create(py$this_output_dir, showWarnings = F, recursive = T)
py_run_string("this_figure_dir = this_output_dir + '/figures'")
py_run_string("sc.settings.figdir = this_figure_dir")
qc_state = "before_QC"
...
tryCatch({
  ...
  py_run_string(paste0(adata_str, " = ad.read_h5ad('/home/haoy/projects/MetaStudiesAnalysis/Integration/Output/Integration/v67_gauss/HFGland_2000_hvgs_50_pcs_Karl/clustering.h5ad')"))
  py_run_string(paste0(adata_str, ".uns['log1p'] = None"))
  py_run_string(paste0("del ", adata_str, ".uns['log1p']"))
...
for(used_table in c("top_p_by_score", "top_p_by_FC", "top_p_by_P")){
  ...
  if(length(top_DEGs) <= max_gene_number){
    ...
    py_run_string(paste0("dot_fig = plot_dotplot_with_annotations(", adata_str, ", var_names=DEG_dict, groupby='", DEG_key, "', layer='normalized', dendrogram=True, n_genes=10, standard_scale = 'var', swap_axes = False, highlight_genes = DEG_dict, highlight_params = {'facecolor': 'yellow', 'edgecolor': 'none', 'alpha': 0.5}, highlight_cluster_line = False, cluster_line_highlight_params = {'color': 'lightgreen', 'alpha': 0.2, 'zorder': -5},)"))
    ...
    #  py_run_string(paste0("dot_fig = plot_dotplot_with_annotations(", adata_str, "[", adata_str, ".obs['StudyID'] == '", this_study, "'], var_names=DEG_dict, groupby='", DEG_key, "', layer='normalized', dendrogram=False, n_genes=10, standard_scale = 'var', swap_axes = False, highlight_genes = DEG_dict, highlight_params = {'facecolor': 'yellow', 'edgecolor': 'none', 'alpha': 0.5}, highlight_cluster_line = False, cluster_line_highlight_params = {'color': 'lightgreen', 'alpha': 0.2, 'zorder': -5},)"))
    ...
    for(ii in 1:ceiling(length(top_DEGs) / max_gene_number)){
      ...
      py_run_string(paste0("dot_fig = plot_dotplot_with_annotations(", adata_str, ", var_names=tmp_genes_ordered, groupby='", DEG_key, "', layer='normalized', dendrogram=True, n_genes=10, standard_scale = 'var', swap_axes = False, highlight_genes = DEG_dict, highlight_params = {'facecolor': 'yellow', 'edgecolor': 'none', 'alpha': 0.5}, highlight_cluster_line = False, cluster_line_highlight_params = {'color': 'lightgreen', 'alpha': 0.2, 'zorder': -5},)"))
      ...
      #for(this_study in eval(parse(text = paste0("unique(as.character(py$", adata_str, "$obs[['StudyID']]))")))){
      #  py_run_string(paste0("dot_fig = plot_dotplot_with_annotations(", adata_str, "[", adata_str, ".obs['StudyID'] == '", this_study, "'], var_names=tmp_genes_ordered, groupby='", DEG_key, "', layer='normalized', dendrogram=False, n_genes=10, standard_scale = 'var', swap_axes = False, highlight_genes = DEG_dict, highlight_params = {'facecolor': 'yellow', 'edgecolor': 'none', 'alpha': 0.5}, highlight_cluster_line = False, cluster_line_highlight_params = {'color': 'lightgreen', 'alpha': 0.2, 'zorder': -5},)"))
...
py_run_string(paste0("sc.pl.embedding(", adata_str, ", '", visualization_embedding, "', color=['", paste(clustering_keys, collapse = "', '"), "'] + ['leiden_2_100'] + annotation_tmp, legend_loc='on data', legend_fontsize=6, size=dotsize, ncols=8, frameon=False, save='_clustering.pdf')"))
for(ii in c(py$factors, "phase", clustering_keys)){
...
#
py_run_string(paste0("sc.pl.embedding(", adata_str, ",
                                      basis='", visualization_embedding, "',
...
py_run_string(paste0("dot_fig = plot_dotplot_with_annotations(", adata_str, ", var_names=marker_dict, groupby='", DEG_key, "', layer='normalized', dendrogram=False, n_genes=10, standard_scale = 'var', swap_axes = False, highlight_genes = marker_dict, highlight_params = {'facecolor': 'yellow', 'edgecolor': 'none', 'alpha': 0.5}, highlight_cluster_line = False, cluster_line_highlight_params = {'color': 'lightgreen', 'alpha': 0.2, 'zorder': -5},)"))
py_run_string(paste0("dot_fig.savefig(f'{this_figure_dir}/dotplot_", adata_str, "_", DEG_key, "_var.pdf', bbox_inches = 'tight')"))
#
